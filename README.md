...
- Phase 4부터는 repo-aware story planner가 work unit, expected artifact, handoff contract, verification ordering까지 반영합니다.
...
- `generated-harness/<slug>/references/artifact-contracts.md`: work unit별 expected artifact contract
- `generated-harness/<slug>/references/handoff-protocol.md`: `_workspace` handoff naming과 역할 전파 규약
- `generated-harness/<slug>/references/verification-policy.md`: verification strategy와 command ordering policy
- `active-harness.json`: 새 feature lane에 적용될 기본 harness snapshot
- `runs/*/loop-state.json`: run status, repeated failure, reopen target, reopen history, guidance focus
...
- repo-aware story split and verification ordering
- generic vs repo-aware planning eval
...
- `npm run smoke:live`는 기본적으로 skip되며, 실제 실행은 `HARNESS_LIVE_SMOKE=1 npm run smoke:live`처럼 opt-in으로만 수행합니다.
